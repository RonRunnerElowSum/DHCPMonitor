...
function PunchIt {
    ...
    do{
        ...
        $BlacklistedMACAddresses | ForEach-Object {
            if($MACsDiscoveredInLogs | Select-String "$_"){
                ...
                try{
                    ...
                    Remove-DhcpServerv4Lease -ComputerName $([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName) -IPAddress $IPToRevoke
                }
                catch{
                    Write-Warning "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor failed to revoke the lease...address immediately!`r`n`r`n$BlacklistedMACFoundLogs`r`n`r`nRevoke lease error:`r`n$($global:intErr++)Error #:$global:intErr`r`n$Error$($Error.Clear())"
                    Write-MSPLog -LogSource "MSP DHCP Monitor" -LogType "Warning" -LogMessage "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor failed to revoke the lease...address immediately!`r`n`r`n$BlacklistedMACFoundLogs`r`n`r`nRevoke lease error:`r`n$($global:intErr++)Error #:$global:intErr`r`n$Error$($Error.Clear())"
                    $Recipients | ForEach-Object {
                        Send-Alert -ToAddress $_ -FromAddress $SenderAddress -Subject "($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device!" -Body "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor failed to revoke the lease...address immediately!`r`n`r`n$BlacklistedMACFoundLogs`r`n`r`nRevoke lease error:`r`n$($global:intErr++)Error #:$global:intErr`r`n$Error$($Error.Clear())"
                    }
                }
                Write-Host "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor successfiully revoked the lease!`r`n`r`n$BlacklistedMACFoundLogs"
                Write-MSPLog -LogSource "MSP DHCP Monitor" -LogType "Information" -LogMessage "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor successfiully revoked the lease!`r`n`r`n$BlacklistedMACFoundLogs"
                $Recipients | ForEach-Object {
                    Send-Alert -ToAddress $_ -FromAddress $SenderAddress -Subject "($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and successfully revoked it!" -Body "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) leased an IP to a blacklised device and the DHCP Monitor successfiully revoked the lease!`r`n`r`n$BlacklistedMACFoundLogs"
                }
            }   
        }
        if($TodaysDHCPLogs | Where-Object {$_.ID -eq 14}){
            ...
            Write-Host "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has run out of DHCP leases...address immediately!`r`n`r`n$EmptyDHCPPoolLogs"
            Write-MSPLog -LogSource "MSP DHCP Monitor" -LogType "Warning" -LogMessage "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has run out of DHCP leases...address immediately!`r`n`r`n$EmptyDHCPPoolLogs"
            $Recipients | ForEach-Object {
                Send-Alert -ToAddress $_ -FromAddress $SenderAddress -Subject "($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has run out of DHCP leases!" -Body "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has run out of DHCP leases...address immediately!`r`n`r`n$EmptyDHCPPoolLogs"
            }
        }
        if($TodaysDHCPLogs | Where-Object {$_.ID -eq 61}){
            ...
            Write-Host "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network that belongs to the AD domain...address immediately!`r`n`r`n$RogueADDHCPServerLogs"
            Write-MSPLog -LogSource "MSP DHCP Monitor" -LogType "Warning" -LogMessage "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network that belongs to the AD domain...address immediately!`r`n`r`n$RogueADDHCPServerLogs"
            $Recipients | ForEach-Object {
                Send-Alert -ToAddress $_ -FromAddress $SenderAddress -Subject "($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server!" -Body "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network that belongs to the AD domain...address immediately!`r`n`r`n$RogueADDHCPServerLogs"
            }
        }
        if($TodaysDHCPLogs | Where-Object {$_.ID -eq 62}){
            ...
            Write-Host "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network...address immediately!`r`n`r`n$RogueDHCPServerLogs"
            Write-MSPLog -LogSource "MSP DHCP Monitor" -LogType "Warning" -LogMessage "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network...address immediately!`r`n`r`n$RogueDHCPServerLogs"
            $Recipients | ForEach-Object {
                Send-Alert -ToAddress $Recipients -FromAddress $SenderAddress -Subject "($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server!" -Body "The DHCP server ($([System.Net.Dns]::GetHostByName($Env:ComputerName).HostName)) has found another DHCP server on the network...address immediately!`r`n`r`n$RogueDHCPServerLogs"
            }
        }
